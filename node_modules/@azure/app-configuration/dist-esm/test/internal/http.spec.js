...
describe("http request related tests", function () {
    ...
    describe("request/reply tests for sync token headers", () => {
        ...
        it("policy is setup properly to send sync tokens", async function () {
            ...
            const policyScope = nock(/.*/, {
                ...
                .get(/.*/)
                .reply(418);
            await assertThrowsRestError(async () => client.getConfigurationSetting({
                key: "doesntmatter"
            }), 418);
    ...
    describe("syncToken", async () => {
        it("update sync token", async () => {
            const syncTokens = new SyncTokens();
            syncTokens.addSyncTokenFromHeaderValue("a=value;sn=0");
            const client = new AppConfigurationClient("Endpoint=https://endpoint.azconfig.io;Id=abc;Secret=123", { syncTokens });
            assert.equal(syncTokens["_currentSyncTokens"].size, 1, "Unexpected number of syncTokens before the `update` call");
            client.updateSyncToken("b=value;sn=3");
            assert.equal(syncTokens["_currentSyncTokens"].size, 2, "Unexpected number of syncTokens after the `update` call");
            assert.deepEqual(syncTokens["_currentSyncTokens"].get("a"), { id: "a", value: "value", sequenceNumber: 0 }, "Unexpected object present for key `a`");
            assert.deepEqual(syncTokens["_currentSyncTokens"].get("b"), { id: "b", value: "value", sequenceNumber: 3 }, "Unexpected object present for key `b`");
        });
    });
});
