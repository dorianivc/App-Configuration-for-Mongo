    ...
    it("simulate the service throttling", async () => {
        ...
        try {
            for (let index = 0; index < numberOfSettings; index++) {
                promises.push(client.addConfigurationSetting({
                    key: key + "-" + index,
                    value: "added"
                }, {
            ...
            errorWasThrown = true;
            chai.assert.equal(error.name, "AbortError", "Unexpected error thrown");
        }
        chai.assert.equal(errorWasThrown, true, "Error was not thrown");
    });
});
//# sourceMappingURL=throttlingRetryPolicy.spec.js.map